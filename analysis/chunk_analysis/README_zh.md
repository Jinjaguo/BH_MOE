...
```text
/home/jinjaguo/BH_MOE/OOD_exp/dif_start_end_loc/outputs/chunk_wise
```

一个 task 对应一个子目录。例如原始 strict LIBERO 判定数据：

```text
/home/jinjaguo/BH_MOE/OOD_exp/dif_start_end_loc/outputs/chunk_wise/put_the_cream_cheese_on_the_plate
```

soft success baseline 复跑结果也会保存成新的 task 目录，例如：

```text
/home/jinjaguo/BH_MOE/OOD_exp/dif_start_end_loc/outputs/chunk_wise/put_the_cream_cheese_on_the_plate_soft_success
```

causal intervention 复跑结果单独保存到 intervention 根目录。注意当前路径名沿用项目里的拼写 `intervetion`：

```text
/home/jinjaguo/BH_MOE/OOD_exp/dif_start_end_loc/outputs/intervetion/<task_name>/chunk<id>
```

例如默认替换 chunk 2：

```text
/home/jinjaguo/BH_MOE/OOD_exp/dif_start_end_loc/outputs/intervetion/put_the_cream_cheese_on_the_plate/chunk2
```
...
rollouts_soft_success.jsonl   # soft-success runner 或当前 causal intervention runner 会生成
```
...
```text
/home/jinjaguo/BH_MOE/OOD_exp/dif_start_end_loc/outputs/videos
```

soft success stopper 视频：

```text
/home/jinjaguo/BH_MOE/OOD_exp/dif_start_end_loc/outputs/videos/soft_success/<task_name>
```

causal intervention 视频：

```text
/home/jinjaguo/BH_MOE/OOD_exp/dif_start_end_loc/outputs/videos/causal_intervention/<task_name>/chunk<id>
```
...
```bash
ROOT=OOD_exp/dif_start_end_loc/outputs/chunk_wise/put_the_cream_cheese_on_the_plate_soft_success
...
/home/jinjaguo/anaconda3/envs/libero/bin/python \
  ...
  --env pi05_libero \
  --port 8000 \
  --trace_root OOD_exp/dif_start_end_loc/outputs/chunk_wise
```
...
```text
OOD_exp/dif_start_end_loc/outputs/chunk_wise/put_the_cream_cheese_on_the_plate_soft_success/trial_<id>/chunk_*.pt
```

输出 trial metadata：

```text
OOD_exp/dif_start_end_loc/outputs/chunk_wise/put_the_cream_cheese_on_the_plate_soft_success/trial_<id>/rollouts_finalize.jsonl
OOD_exp/dif_start_end_loc/outputs/chunk_wise/put_the_cream_cheese_on_the_plate_soft_success/trial_<id>/rollouts_soft_success.jsonl
OOD_exp/dif_start_end_loc/outputs/chunk_wise/put_the_cream_cheese_on_the_plate_soft_success/trial_<id>/rollouts_state_record.jsonl
```
...
```text
OOD_exp/dif_start_end_loc/outputs/videos/soft_success/put_the_cream_cheese_on_the_plate_soft_success/trial<id>_<suffix>.mp4
...
2. causal-intervention soft-success：使用 `08_start_causal_intervention_server.py`。当前推荐 server 的 `--output_task_name` 使用 `chunk<id>`，client 的 `--intervention_chunk` 与 server 一致；如果 client 显式传 `--output_task_name`，它也必须和 server 对齐。
...
当前 intervention 实验统一用 soft success 作为最终成败标签。也就是说，`rollouts_finalize.jsonl` 的顶层 `success` 和 summary 里的 `success/analysis_success` 都表示 soft-success label，而原始 LIBERO strict label 会保存在 `strict_success` 字段里。也就是说引用了10这个文件
...
可以通过 `--intervention_chunk <chunk_id>` 自由切换要替换的 chunk，例如 `chunk0` 到 `chunk5`。输出也会自动按 chunk 分目录保存：

```text
OOD_exp/dif_start_end_loc/outputs/intervetion/<task_name>/chunk<id>
OOD_exp/dif_start_end_loc/outputs/videos/causal_intervention/<task_name>/chunk<id>
```
...
client: 09_run_causal_intervention_rollouts.py
```
...
python analysis/chunk_analysis/08_start_causal_intervention_server.py \
  --source_trace_root OOD_exp/dif_start_end_loc/outputs/chunk_wise \
  --trace_root OOD_exp/dif_start_end_loc/outputs/intervetion/put_the_cream_cheese_on_the_plate \
  --task_name put_the_cream_cheese_on_the_plate \
  --output_task_name chunk0 \
  --intervention_chunk 0 \
  --state action_head_input \
  --port 8001
```
...
| 参数 | 含义 |
|---|---|
| `--source_trace_root` | 读取 baseline success reference 的根目录，通常是 `OOD_exp/dif_start_end_loc/outputs/chunk_wise` |
| `--trace_root` | 保存 intervention traces 的根目录，当前推荐 `OOD_exp/dif_start_end_loc/outputs/intervetion/<task_name>` |
| `--task_name` | baseline task 名，用于读取 success reference |
| `--output_task_name` | intervention 结果保存到哪个子目录，默认推荐 `chunk<id>` |
...
```text
OOD_exp/dif_start_end_loc/outputs/intervetion/<task_name>/chunk<id>/intervention_manifest.json
OOD_exp/dif_start_end_loc/outputs/intervetion/<task_name>/chunk<id>/trial_<id>/chunk_*.pt
```
...
### 9.2 运行 soft-success causal rollout client
...
4. 用 soft success 作为最终 success/failure 标签。
5. 保存 rollout metadata、server hidden-state chunks、视频和 summary。

如果不传 `--trial_ids`，脚本会默认跑 task 里所有失败的原始 rollout。`--num_trials` 现在只是可选上限；例如 `--num_trials 20` 表示只取前 20 个失败 trial。
...
```text
OOD_exp/dif_start_end_loc/outputs/intervetion/put_the_cream_cheese_on_the_plate/chunk2/trial_<id>/rollouts_finalize.jsonl
OOD_exp/dif_start_end_loc/outputs/intervetion/put_the_cream_cheese_on_the_plate/chunk2/trial_<id>/rollouts_soft_success.jsonl
OOD_exp/dif_start_end_loc/outputs/intervetion/put_the_cream_cheese_on_the_plate/chunk2/trial_<id>/chunk_*.pt
```

输出视频：

```text
OOD_exp/dif_start_end_loc/outputs/videos/causal_intervention/put_the_cream_cheese_on_the_plate/chunk2/source_trial<id>_intervention_<strict_success_or_soft_success_or_failure>.mp4
OOD_exp/dif_start_end_loc/outputs/videos/causal_intervention/put_the_cream_cheese_on_the_plate/chunk2/intervention_summary.csv
```

summary 里关键列：

```text
source_trial_id       原始失败 trial id
intervention_trial_id 复跑 trial id，默认等于 source_trial_id
intervention_chunk    被替换的 chunk id
success               最终分析 success，等于 analysis_success
strict_success        LIBERO/BDDL strict success
soft_success          soft evaluator success；strict success 自动算 soft success
analysis_success      当前用于分析的最终标签，等于 soft_success
soft_success_only     strict 失败但 soft 成功
true_failure          strict 和 soft 都失败
stop_reason           strict_success / soft_success / timeout
```

### 9.3 跑不同 chunk 的 intervention

每次 server 和 client 的 `--intervention_chunk` 必须一致，同时 server 的 `--output_task_name` 建议改成对应的 `chunk<id>`。
...
```text
OOD_exp/dif_start_end_loc/outputs/intervetion/put_the_cream_cheese_on_the_plate/chunk0
OOD_exp/dif_start_end_loc/outputs/videos/causal_intervention/put_the_cream_cheese_on_the_plate/chunk0
```
...
```bash
ROOT=OOD_exp/dif_start_end_loc/outputs/intervetion/put_the_cream_cheese_on_the_plate/chunk2
OUT=analysis/chunk_analysis/results/put_the_cream_cheese_on_the_plate_intervention_chunk2
PY=/home/jinjaguo/anaconda3/envs/libero/bin/python
...
如果重新跑 causal intervention，`09_run_causal_intervention_rollouts.py` 默认会删除对应 trial 目录再跑，防止旧 chunk 文件污染。需要保留旧目录时再显式加 `--keep_existing_trial_dir`。
...
| baseline hidden chunks | `/home/jinjaguo/BH_MOE/OOD_exp/dif_start_end_loc/outputs/chunk_wise/put_the_cream_cheese_on_the_plate` |
| soft-success hidden chunks | `/home/jinjaguo/BH_MOE/OOD_exp/dif_start_end_loc/outputs/chunk_wise/put_the_cream_cheese_on_the_plate_soft_success` |
| causal intervention hidden chunks | `/home/jinjaguo/BH_MOE/OOD_exp/dif_start_end_loc/outputs/intervetion/put_the_cream_cheese_on_the_plate/chunk2` |
...
| causal videos | `/home/jinjaguo/BH_MOE/OOD_exp/dif_start_end_loc/outputs/videos/causal_intervention/put_the_cream_cheese_on_the_plate/chunk2` |
| soft-success videos | `/home/jinjaguo/BH_MOE/OOD_exp/dif_start_end_loc/outputs/videos/soft_success` |
...
2. 使用 `09_run_causal_intervention_rollouts.py` 连接 causal server，按 soft success stopper 复跑所有原始失败 trial。
3. 确保输出目录是：

```text
OOD_exp/dif_start_end_loc/outputs/intervetion/put_the_cream_cheese_on_the_plate/chunk2
OOD_exp/dif_start_end_loc/outputs/videos/causal_intervention/put_the_cream_cheese_on_the_plate/chunk2
```

4. 用 `01-07` 对 causal intervention 结果重新跑分析。
